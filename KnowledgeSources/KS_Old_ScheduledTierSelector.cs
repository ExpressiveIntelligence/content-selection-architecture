...
namespace CSA.KnowledgeSources
{
    ...
    public class KS_Old_ScheduledTierSelector : KS_Old_ScheduledFilterSelector
    {
        ...
        public KS_Old_ScheduledTierSelector(IBlackboard blackboard, string tierSlot) : base(blackboard, DefaultOutputPoolName, GenerateHasMetadataSlotFilter(tierSlot))
        {
            TierSlot = tierSlot;
        }

        public KS_Old_ScheduledTierSelector(IBlackboard blackboard, string outputPool, string tierSlot) : base(blackboard, outputPool, GenerateHasMetadataSlotFilter(tierSlot))
        {
            TierSlot = tierSlot;
        }

        public KS_Old_ScheduledTierSelector(IBlackboard blackboard, string inputPool, string outputPool, string tierSlot) : 
            base(blackboard, inputPool, outputPool, GenerateHasMetadataSlotFilter(tierSlot))
        {
            TierSlot = tierSlot;
        }

        public KS_Old_ScheduledTierSelector(IBlackboard blackboard, string outputPool, FilterCondition filter, string tierSlot) : 
            base(blackboard, outputPool, (ContentUnit cu) => filter(cu) && GenerateHasMetadataSlotFilter(tierSlot)(cu))
        {
            TierSlot = tierSlot;
        }

        public KS_Old_ScheduledTierSelector(IBlackboard blackboard, string inputPool, string outputPool, FilterCondition filter, string tierSlot)
