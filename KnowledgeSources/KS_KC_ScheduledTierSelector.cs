...
namespace CSA.KnowledgeSources
{
     ...
     */
    public abstract class KS_KC_ScheduledTierSelector<T> : KS_KC_ScheduledFilterSelector where T : KnowledgeComponent, IComparable
    {
        ...
        protected KS_KC_ScheduledTierSelector(IBlackboard blackboard, string outputPool) : base(blackboard, outputPool, GenerateHasComponent<T>())
        {
        }

        protected KS_KC_ScheduledTierSelector(IBlackboard blackboard, string inputPool, string outputPool) :
            base(blackboard, inputPool, outputPool, GenerateHasComponent<T>())
        {
        }

        protected KS_KC_ScheduledTierSelector(IBlackboard blackboard, string outputPool, FilterCondition filter) :
            base(blackboard, outputPool, (Unit u) => filter(u) && GenerateHasComponent<T>() (u))
        {
        }

        protected KS_KC_ScheduledTierSelector(IBlackboard blackboard, string inputPool, string outputPool, FilterCondition filter)
